...
export interface EditModeComponentProps {
    ...
    confirmButtonPropsConfig?: (props: React.ComponentProps<typeof EditButton>) => Partial<React.ComponentProps<typeof EditButton> & DataAttributes>;
    /** Конфиг функция пропсов для кнопки отмены редактирования. На вход получает начальный набор пропсов, на
     * выход должна отдавать объект с пропсами, которые будут внедряться после оригинальных пропсов. */
    cancelButtonPropsConfig?: (props: React.ComponentProps<typeof EditButton>) => Partial<React.ComponentProps<typeof EditButton> & DataAttributes>;
    /** Конфиг функция пропсов для кнопки начала редактирования. На вход получает начальный набор пропсов, на
     * выход должна отдавать объект с пропсами, которые будут внедряться после оригинальных пропсов. */
    editButtonPropsConfig?: (props: React.ComponentProps<typeof EditIcon>) => Partial<React.ComponentProps<typeof EditIcon> & DataAttributes>;
}
export {};
